...
import edu.kit.iti.sfc.extree.SymEx;
        ...
        for (Variable xs : x) {
            if (xs.isIn()) {
                if (xs.getType().equals(Vartype.BOOL)) {
                    result.put(xs.getName() + SymEx.TIME_SEPERATOR + k,
                            ctx.mkBoolConst(xs.getName() + SymEx.TIME_SEPERATOR + k));
                }
                if (xs.getType().equals(Vartype.INT)) {
                    result.put(xs.getName() + SymEx.TIME_SEPERATOR + k,
                            ctx.mkIntConst(xs.getName() + SymEx.TIME_SEPERATOR + k));
                }
            } else {
                if (xs.getType().equals(Vartype.BOOL)) {
